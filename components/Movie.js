...
function Movie(props) {
  ...
  return (
    <div className={styles.card}>
      <img className={styles.image} src={props.poster} alt={props.title} style={{width:'100%'}} />
      <div className={styles.textContainer}>
        <div>
          <span className={styles.name}>{props.title}</span>
          <p className={styles.description}>{props.overview.length <= 250 ? props.overview : `${props.overview.slice(0,247)}...`}</p>
        </div>
